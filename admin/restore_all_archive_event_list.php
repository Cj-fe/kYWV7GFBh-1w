...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $deletedAlumniData = $firebase->retrieve("deleted_event");
        ...
        if ($deletedAlumni) {
            foreach ($deletedAlumni as $id => $alumni) {
                ...
                $insertResponse = $firebase->update("event", $id, $alumni);
                ...
                if (!$insertResponse || !$deleteResponse) {
                    echo json_encode(['status' => 'error', 'message' => 'Failed to retrieve some event records.']);
                    exit;
                }
            }
            echo json_encode(['status' => 'success', 'message' => 'All event records retrieved successfully.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'No event records to retrieve.']);
        }
    } catch (Exception $e) {
        // Log the exception message
        error_log('Error retrieving event records: ' . $e->getMessage());
